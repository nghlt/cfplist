const plist = (() => {
  const DataType = {
    ...
    parse = (plistStr) => {
      function numOccurrences(reg, text) {
        let matches = text.match(reg);
        return matches ? matches.length : 0;
      }
      function parser(keyval) {
        ...
        if (matchingKeyVal) {
          ...
          const regexKeyBooleanNull = /^<key>([^>]+)<\/key><(\w+)\/>$/,
            matchingKeyBooleanNull = regexKeyBooleanNull.exec(keyval);
          if (matchingKeyBooleanNull) {
      ...
      plistStr = plistStr.replace(/<\!--.*-->/g, "").replace(/(^\s+|\s+$|\n+)/gm, "");
      const regexRoot = /^(.*<plist[^>]*>|)\s*<(dict|array)>/;
      const matchingRoot = regexRoot.exec(plistStr);
      if (!matchingRoot) return null;

      let isRootAnArray = matchingRoot[2] === "array" ? true : false,
        adjustedPayload = [];
      let payload = plistStr
        .replace(regexRoot, "")
        .replace(/(<\/dict>|<\/array>)[\s\n]*(|<\/plist>)$/, "")
        .replace(/(<(\/\w+|(\w+)\/)>)/g, "$1\n")
        .replace(/(<\/key>)(\n|\s)*/g, "$1")
        .replace(/(\n|\s)*(<\/dict>|<\/array>)/g, "$2");
